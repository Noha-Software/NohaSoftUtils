...
namespace NohaSoftware.Utilities
{
    ...
    public class SerializableTuple<T1, T2>
    {
        ...
        public static implicit operator KeyValuePair<T1, T2>(SerializableTuple<T1, T2> serializableTuple)
        {
	...
	public class SerializableTuplePropertyDrawer : PropertyDrawer
	{
		public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
		{
			...
			float keyHeight = EditorGUI.GetPropertyHeight(property.FindPropertyRelative("Key"), includeChildren: true);
			float valueHeight = EditorGUI.GetPropertyHeight(property.FindPropertyRelative("Value"), includeChildren: true);
			float spacing = EditorGUIUtility.standardVerticalSpacing;

			// Draw the key and value fields
			EditorGUI.PropertyField(new Rect(position.x, position.y, position.width / 2 - spacing, keyHeight), property.FindPropertyRelative("Key"), GUIContent.none);
			EditorGUI.PropertyField(new Rect(position.x + position.width / 2, position.y, position.width / 2 - spacing, valueHeight), property.FindPropertyRelative("Value"), GUIContent.none);
		...
		public override float GetPropertyHeight(SerializedProperty property, GUIContent label)
		{
			float keyHeight = EditorGUI.GetPropertyHeight(property.FindPropertyRelative("Key"), includeChildren: true);
			float valueHeight = EditorGUI.GetPropertyHeight(property.FindPropertyRelative("Value"), includeChildren: true);
			float lineHeight = Mathf.Max(EditorGUIUtility.singleLineHeight, keyHeight);
			lineHeight = Mathf.Max(lineHeight, valueHeight);
			return lineHeight;
		}
	}
#endif
